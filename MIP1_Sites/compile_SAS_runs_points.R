...
#
#
# Original ED SAS solution Script:
...
base  <- "/projectnb/dietzelab/paleon/ED_runs/phase1a_spininitial.v3/"
out   <- "/projectnb/dietzelab/paleon/ED_runs/SAS_spinup/phase1a_spinup.v3/"

# Steps used to describe the steady state
blckyr  <- 25 #number of years to chunk data by
nsteps  <- (2100-1850)/blckyr # The number of blocks = the number steps we'll have
...
for(s in sites){
  ...
  yrs <- seq(yeara, yearz, by=blckyr) # We're paramterizing with the upper limit for each bin, so we're starting with a 10-year old forest
  ...
  colnames(pss.big) <- c("site","year","patch","dst","age","area","water","fsc","stsc","stsl",
	...
	for(y in met.yra:met.yrz){
      ...
      for(m in month.begin:month.end){
        ...
        file.now    <- paste(s,"spin","-E-",year.now,"-",month.now,"-",day.now,"-"
                             ,hour.now,"-",sufx,sep="")
  ...
  #---------------------------------------
    ...
    # ssc_ss <- sum(ssc_in_y*area.dist)/(ssc_loss * A_decomp ) # Slow soil C -- NEEDS TO BE ADJUSTED FOR steady-state SSL since ssc_in  = ssl_loss*(1-r_stsc)
